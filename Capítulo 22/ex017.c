...
int main()
{
    ...
    // Carregue automaticamente a lista ligada do arquivo quando o programa inicia
    load();
...
// Função para salvar a lista ligada em um arquivo
void save(void) {
    FILE *file;
    if ((file = fopen("linked_list.dat", "wb")) == NULL) {
    ...
    current = first;
    while (current != NULL) {
        fwrite(current, sizeof(struct typical), 1, file);
        current = current->next;
    }
...
// Função para carregar a lista ligada de um arquivo
void load(void) {
    FILE *file;
    struct typical *loaded_record;

    if ((file = fopen("linked_list.dat", "rb")) == NULL) {
    ...
    while (!feof(file)) {
        loaded_record = create();
        if (fread(loaded_record, sizeof(struct typical), 1, file) == 1) {
            if (first == NULL) {
                first = loaded_record;
                current = first;
            } else {
                current->next = loaded_record;
                current = current->next;
            }
        }
    }
